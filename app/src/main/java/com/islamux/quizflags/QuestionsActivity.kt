...
class QuestionsActivity : AppCompatActivity() , View.OnClickListener{
    ...
    override fun onClick(v: View?) {
        ...
        when (v?.id) {
            binding.tvOptionOne.id -> {
                ...
                Log.d(TAG, "Fathi onClick option 1")

            }

            binding.tvOptionTwo.id -> {
                selectedOptionView(binding.tvOptionTwo, 2)
                Log.d(TAG, "Fathi onClick option 2")
            }

            binding.tvOptionThree.id -> {
                selectedOptionView(binding.tvOptionThree, 3)
                Log.d(TAG, "Fathi onClick option 3")
            }

            binding.tvOptionFour.id -> {
                selectedOptionView(binding.tvOptionFour, 4)
                Log.d(TAG, "Fathi onClick option 4")
            }

            binding.submitButton.id -> {

                Log.d(TAG, "Fathi onClick Submit Button")
                ...
                if (currentPosition == questionsList!!.size) {
                    ...
                    binding.submitButton.text = getString(R.string.go_to_next_question)
                }
    ...
    private fun setQuestion() {
        ...
        binding.tvProgress.text = "$currentPosition" + "/" + binding.progressBar.getMax()
